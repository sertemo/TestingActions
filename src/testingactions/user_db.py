...
class DataBase:
    ...
    def add(self, user:User, collection:str) -> None:
        if collection not in self.db:
            raise AttributeError(f"La colección {collection} no existe. Debes crearla primero con create_collection()")
...
def inactivate_user(db_conn:DataBase, collection:str, username:str) -> dict | None:
    user = db_conn.find_one(username, collection)
    if user is not None:
        user.state = UserState.inactive
        return user.__dict__
    
def activate_user(db_conn:DataBase, collection:str, username:str) -> dict | None:
    user = db_conn.find_one(username, collection)
    if user is not None:
        user.state = UserState.active
        return user.__dict__
    
def block_user(db_conn:DataBase, collection:str, username:str) -> dict | None:
    user = db_conn.find_one(username, collection)
    if user is not None:
        user.state = UserState.blocked
        return user.__dict__

def create_user(username:str, edad:int, password:str, *, db_conn:DataBase, collection:str) -> dict:
    if user_already_exists(username, db_conn, collection):
        raise ValueError(f"El usuario {username} ya existe en la colección {collection}")
    if not is_valid_password(password):
        raise ValueError(f"El password no es válid. Debe tener al menos 4 dígitos.")
    if not username_is_not_one_word(username):
        raise ValueError(f"El username no puede contener espacios")
    user = User(username, edad, password)
...
def user_already_exists(username:str, db_conn:DataBase, collection:str) -> bool:
    if db_conn.find_one(username, collection):
        return True
    else:
        return False
